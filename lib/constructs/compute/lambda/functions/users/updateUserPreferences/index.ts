...
import { UserPreferences, UpdateUserPreferencesInput } from '../../../types/generated'
import { UpdateUserPreferencesEvent, UpdateUserPreferencesHandler } from '../../../types/events'
 ...
 */
export const handler: UpdateUserPreferencesHandler = async (
  event: UpdateUserPreferencesEvent
): Promise<UserPreferences> => {
  console.log(
    'UpdateUserPreferences Lambda invoked:',
  ...
  try {
    ...
    if (error.name === 'ConditionalCheckFailedException') {
      throw new Error(
        'Preferences update failed - user may have been deleted. Please try again.'
      )
    }
...
function validateInput(input: UpdateUserPreferencesInput): void {
  ...
  if (
    ...
    throw new Error(
      'At least one preference field must be provided for update.'
    )
  }
  ...
  if (input.locationSharing !== undefined) {
    ...
    if (input.locationSharing && !validLocationSettings.includes(input.locationSharing)) {
      throw new Error(
        'Location sharing must be one of: ALWAYS, HUBS_ONLY, NEVER'
      )
    }
  }
  ...
  if (input.profileVisibility !== undefined) {
    ...
    if (input.profileVisibility && !validVisibilitySettings.includes(input.profileVisibility)) {
      throw new Error(
        'Profile visibility must be one of: PUBLIC, FRIENDS, PRIVATE'
      )
    }
  }
}
