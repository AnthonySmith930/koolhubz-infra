...
import { User, UserProfile, UpdateProfileEvent, UpdateProfileInput } from '../../../types/userTypes';
 ...
 */
export const handler = async (event: UpdateProfileEvent): Promise<UserProfile> => {
  ...
  try {
    ...
    console.log(`Updating profile for user: ${userId}, auth: ${auth.authMethod}`);
    ...
    const setExpressions = updateExpressions.filter(expr => !expr.includes('REMOVE'));
    ...
    if (error.name === 'ConditionalCheckFailedException') {
      throw new Error('Profile update failed - user may have been deleted. Please try again.');
    }
...
function validateInput(input: UpdateProfileInput): void {
  ...
  if (input.displayName === undefined && input.bio === undefined) {
    throw new Error('At least one field (displayName or bio) must be provided for update.');
  }
