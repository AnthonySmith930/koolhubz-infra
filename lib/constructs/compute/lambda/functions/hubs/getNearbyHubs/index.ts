...
export const handler: GetNearbyHubsHandler = async (event: GetNearbyHubsEvent): Promise<Hub[]> => {
  ...
  try {
    ...
    const { encode: geohashEncode, neighbors: geohashNeighbors } =
      ngeohash.default
    ...
    console.log(
      `Searching for hubs within ${radiusKm}km of (${input.latitude}, ${input.longitude})`
    )
    ...
    console.log(
      `User geohash: ${userGeohash}, using precision ${precision}, prefix: ${userGeohashPrefix}`
    )

    // Get neighboring geohash prefixes to handle boundary cases
    const searchPrefixes = [
      userGeohashPrefix,
      ...geohashNeighbors(userGeohashPrefix)
    ]
    console.log(
      `Searching ${searchPrefixes.length} geohash prefixes:`,
      searchPrefixes
    )
    ...
    console.log(
      `After distance filtering: ${hubsWithDistance.length} nearby hubs`
    )
